WITH u AS (SELECT * FROM {{ ref("dim_user") }} 
), ue AS (SELECT * FROM {{ ref("fact_user_event") }} 
) SELECT 
variant_id,ue.user_id,datestamp,age,gender,
...
FROM ue LEFT JOIN u ON ue.user_id = u.user_id GROUP by 1, 2, 3, 4, 5 
